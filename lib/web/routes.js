...
module.exports.install = function(app, status_api, devops){
  ...
  app.get(
    ...
    var cutoff = new Date(now.valueOf() - month);

    var notes = [];

    var options = _.bind(utils.github_request_options, null, get_devops(req, devops, 'github'));
    var next = options();
    ...
    async.whilst(
        ...
        utils.request_maker(next, function(errors, results){
          ...
          _.each(pulls, function(pull){
            if (new Date(pull.created_at) >= cutoff){
              notes.push([pull.title, pull.body]);
            }else{
              reached_cutoff = true;
            }
          });
      ...
      function(err){
        ...
        res.render('release_notes.jade', {notes: notes, errors: err});
      }
    );
  }),
  app.get(
    ...
    function(req, res) {
      ...
      _.each(future_events, function(event){
        event.days_remaining = Math.floor((timestamp - now) / (1000*60*60*24));
        // figure out the amount and offset from the right 100px and from the left 25px
        var amt = ((timestamp - now) / (max - now + 1)) * 0.8;
