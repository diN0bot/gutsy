...
module.exports = utils.create_middleware('version_one', function(req, res, next, payload, api_config) {
  ...
  utils.request_maker(options,
    function(error, data){
      ...
      var ageUnknown = 'defects of unknown age (wtf?)';
      byAge[age0] = defDict();
      byAge[age1] = defDict();
      byAge[age2] = defDict();
      byAge[age3] = defDict();
      byAge[age4] = defDict();
      byAge[ageUnknown] = defDict();
      var type = age0;
      ...
      if (!_.isEmpty(data.error)){
        ...
        _.each (assets, function(asset) {
          ...
          if (diff <= 1) { type = age0; }
          else if (diff <= 5 && diff > 1) { type = age1; }
          else if (diff <= 10 && diff > 5){ type = age2; }
          else if (diff <= 30 && diff > 10){ type = age3; }
          else if (diff > 30){ type = age4; }
          else { type = ageUnknown; }
