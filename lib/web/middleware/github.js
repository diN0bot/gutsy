...
var Cache = function(name, expires, work_function){
  //TODO: assert name, expires, work_function, work_function is callable
  module.exports[name] = function(req, res, next){
    // TODO: this stampedes
    var expires_on = Date.now() + expires / 1000;
    if (name in req.app._cache && req.app._cache[name].expires < expires_on ){
      req.devops[name] = req.app._cache[name]['payload'];
      next();
    } else{
      var data = work_function(req, res);
      // TODO: add timestamp here
      req.app._cache[name] = {expires_on: now, payload: data};
    }
  };
};
 ...
 */
module.exports = function _github(req, res, next) {
