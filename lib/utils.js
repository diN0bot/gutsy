...
var api_host_properties = exports.make_class({
  ...
  * @param {data} stats for a given host (see link above)
 */
  init: function(data){
...
/** A representation of the stats for all hosts.  This data is flushed to disk every settings.flush_stats_interval
 *
 */
exports.api_cache = exports.make_class({
  init: function(){
    ...
    try{
      ...
      if (e.code !== 'ENOENT'){
        console.warn('Couldn\'t load flushed data');
        throw e;
      }
    }
    if (data !== null){
    ...
    // set up the flusher
    this.interval_id = setInterval(_.bind(this._flush_data, this), this._flush_interval);
  },
  ...
  * @param {object} host_data the given data for a host (see link in api_host_properties docs)
  ...
  _projects: {},
  // where to dump the stats every so _flush_interval ms
  _flush_path: path.join(__dirname, "_service_stats.json"),
  _flush_interval: settings.flush_stats_interval || 5 * 1000
});
