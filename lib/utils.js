...
exports.get_v1_options_from_branch_name = function(v1_config, name, selection){
  var options, path, v1_spec, match, identifier, type;

  if (!selection){
    selection = "Description";
  }

  match = name.match(/([B|D])[ -_]?([0-9]{4,7})/);
  ...
  path = ['/',
    ...
    "/rest-1.v1/Data/",
    type === "B" ? "Story" : "Defect",
    util.format("?sel=%s&where=Number='%s'", selection, identifier)
  ];
