...
module.exports = function github(req, res, next) {
  ...
  async.parallel({
    ...
    function(errors, results){
      var _github = req.devops.github;
      _github.errors = errors;
      for (var key in results){
        var response = results[key];
        try{
          var clean_results = parse_github_response(results);
          _github.response[key] = clean_results;
        } catch(e){
          // TODO: can we get an api error and a message?
          _github.errors[key] = e;
        }
      }
